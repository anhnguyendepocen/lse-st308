...
For the model see the file `LogisticRegression.stan`.
...
y = biopsy$survival # y variable
n = length(y) # number of observations
X = cbind(rep(1,n),subset(biopsy, select = c(age,nodes))) # does the same as above
p = dim(X)[2] # number of variables plus the constant

biopsy_dat=list(n=n, p=p, X=X, y=y)
```

For the model we can use the same stan file as before `LogisticRegression.stan`.

```{r}
fit <- stan(file = 'LogisticRegression.stan', data = biopsy_dat,chains=2,init=0)
```

Finally, summarise theposterior output

```{r}
print(fit)
traceplot(fit)
plot(fit, pars=c("beta[2]","beta[3]"),show_density = TRUE, ci_level = 0.95, fill_color = "green")
```
